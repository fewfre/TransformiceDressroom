		...
		public function colorFur(pSkinPart:DisplayObject, pColor:int):DisplayObject {
			...
			if (pSkinPart.numChildren > 1) {
				while (i < pSkinPart.numChildren) {
					...
					if (tChild.name == "c0") {
						Main.costumes.applyColorToObject(tChild, pColor);
					}
					else if (tChild.name == "c1") {
						Main.costumes.applyColorToObject(tChild, Main.costumes.shamanColor);
					}
					i++;
				}
			} else {
				Main.costumes.applyColorToObject(pSkinPart, pColor);
			}
			return pSkinPart;
		}
